...
const FeatureHighlightSchema = z.object({
  text: z.string().describe('A short, benefit-oriented phrase highlighting a key feature.'),
  icon: z.string().describe('A relevant icon name from the Lucide icon library (e.g., "Rocket", "Shield", "Zap").'),
});
...
const ContentSuggestionOutputSchema = z.object({
  ...
  faqs: z.array(FaqSchema).length(3).describe('An array of 3 frequently asked questions with answers, directly related to the company and its services.'),
  featureHighlights: z.array(FeatureHighlightSchema).length(4).describe('An array of 4 feature highlights with text and a suggested icon name.'),
});
