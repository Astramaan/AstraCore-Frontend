...
function ClientHomePageContent({ params }: { params: { organizationId: string; clientId: string } }) {
    ...
    useEffect(() => {
        if (!loading && user && user.team === 'New User') {
            ...
            router.replace(`/organization/${user.organizationId}/client/lead/${user.userId}/home`);
        }
    }, [user, loading, router]);
