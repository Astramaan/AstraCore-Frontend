...
function getAuthHeaders(req: Request): Record<string, string> {
    ...
    if (!userHeader) {
        console.warn("x-user header is missing in request to /api/check-email");
...
export async function POST(req: Request) {
    try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/v1/check-email`, {
            method: 'POST',
        ...
        console.error("Check email proxy failed:", error);
