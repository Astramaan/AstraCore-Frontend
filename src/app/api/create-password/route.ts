...
function getAuthHeaders(req: Request): Record<string, string> {
  ...
  if (!userHeader) {
    console.warn("x-user header is missing in request to /api/create-password");
...
export async function POST(req: Request) {
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/create-password`, {
      method: "POST",
    ...
    console.error("Create password proxy failed:", error);
