...
function getAuthHeaders(req: Request): Record<string, string> {
  ...
  if (!userHeader) {
    console.warn("x-user header is missing in request to /api/users/teams");
...
export async function GET(req: Request) {
  try {
    const res = await fetch(`${API_BASE_URL}/org/meetings/teams`, {
    ...
    console.error("Get teams proxy failed:", error);
