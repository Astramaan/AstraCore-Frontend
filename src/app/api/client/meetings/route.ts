...
function getAuthHeaders(req: Request): Record<string, string> {
  ...
  if (!userHeader) {
    console.warn("x-user header is missing in request to /api/client/meetings");
...
export async function GET(req: Request) {
  try {
    const res = await fetch(`${API_BASE_URL}/client/meetings`, {
      headers: getAuthHeaders(req),
    });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch (error) {
    console.error("Get client meetings proxy failed:", error);
