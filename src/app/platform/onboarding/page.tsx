...
const TrackingButton = ({ name, completed, active, isRed }: { name: string, completed: boolean, active?: boolean, isRed?: boolean }) => {
    return (
        <div className={`h-14 p-3.5 rounded-[50px] flex-1 flex justify-center items-center gap-2 ${completed ? 'bg-green-600' : active ? (isRed ? 'bg-white outline outline-1 outline-offset-[-1px] outline-red-500' : 'bg-white outline outline-1 outline-offset-[-1px] outline-primary') : 'bg-white outline outline-1 outline-offset-[-1px] outline-stone-300'}`}>
            {completed && <Check className="w-6 h-6 text-white" />}
            <div className={`text-sm lg:text-base font-medium truncate ${completed ? 'text-white' : active ? (isRed ? 'text-red-500' : 'text-primary') : 'text-stone-500'}`}>{name}</div>
        </div>
    );
}

const OnboardingItem = ({ item }: { item: any }) => (
    <div className="flex justify-center items-center gap-4 border-b border-stone-200 last:border-b-0 py-6">
        <div className="w-24 h-32 relative bg-gradient-to-b from-white to-emerald-200 rounded-bl-3xl hidden md:flex justify-center items-center">
            <div className="text-gray-500 text-4xl font-bold">{item.id}</div>
        </div>
        <div className="flex-1 flex flex-col gap-4">
            <div className="w-full inline-flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                ...
                <div className="flex-1 flex justify-end items-center gap-4 md:gap-8 lg:gap-24">
                    <div className="flex items-center gap-4">
                        {item.statusText && <div className="text-right text-red-500 text-lg font-medium">{item.statusText}</div>}
                        <div className="text-right text-gray-500 text-lg font-semibold whitespace-nowrap">{item.completedStages}</div>
                    </div>
                    <div className="text-center text-gray-500 text-lg font-medium hidden md:block">{item.date}</div>
                </div>
            </div>
            <div className="w-full relative flex flex-col md:flex-row justify-center items-start gap-4 md:gap-6">
                <div className="w-full md:flex-1 h-[5px] left-0 top-[29px] absolute bg-gray-100 rounded-[5px] hidden md:block" />
                
                <div className="flex flex-col md:flex-row justify-start items-stretch md:items-center gap-1.5 md:gap-3.5 w-full flex-1">
                    {item.stages.map((stage: any, index: number) => {
                        const isCompletedLine = stage.completed;
                        return (
                            <React.Fragment key={index}>
                                <TrackingButton name={stage.name} completed={stage.completed} active={stage.active} isRed={stage.isRed} />
                            </React.Fragment>
                        )
                    })}
                </div>
                <div className="w-full md:w-auto">
                    <Button className="h-14 w-full md:w-36 px-10 py-3.5 bg-background rounded-[50px] flex justify-center items-center gap-2">
        ...
        <div className="p-4 md:p-8 space-y-6">
             <header className="flex justify-between items-center mb-8">
                <div className="flex justify-start items-center gap-4">
                    ...
                    <div className="w-px h-12 bg-stone-300 hidden md:block"></div>
                    <h1 className="text-black text-2xl md:text-4xl font-bold">Onboarding management</h1>
                </div>
                 <div className="flex justify-end items-center gap-6">
                    <Button variant="ghost" size="icon" className="p-3.5 bg-white rounded-[50px]">
                        <Bell className="relative"/>
                    </Button>
                    <div className="w-px h-12 bg-stone-300/0 hidden md:block"></div>
