  ...
  <div className="space-y-2">
    <Label
      ...
      className={cn(
        ...
        value ? "text-grey-1" : "text-zinc-900 dark:text-white",
      )}
    >
      {label}
    </Label>
...
const FloatingLabelTextarea = ({
  ...
  <div className="space-y-2">
    ...
    <Textarea
      id={id}
      className="h-36 bg-background dark:bg-input rounded-3xl p-4"
      value={value}
      {...props}
    />
  </div>
...
const FileUploadField = ({
  ...
  <div className="space-y-2">
    <Label
      htmlFor={id}
      className={cn(
        "px-2 text-lg whitespace-nowrap",
        fileName ? "text-grey-1" : "text-zinc-900 dark:text-white",
      )}
    >
      {label}
    </Label>
    <div className="flex flex-col md:flex-row md:items-center rounded-xl md:rounded-full border border-stone-300 md:h-[54px] bg-input p-3 md:px-3 gap-2">
      <div className="flex-1 flex items-center justify-between">
        <span className="text-sm text-muted-foreground mr-2 truncate flex-1">
      ...
      <div className="flex items-center gap-1 md:gap-2 self-end md:self-center">
        ...
        <label
          htmlFor={id}
          className="cursor-pointer text-sm text-foreground flex items-center gap-1 pl-1 md:pl-2"
        >
    ...
    <div className="space-y-2">
      ...
      <div className="bg-input rounded-full p-2 flex flex-wrap gap-2 min-h-[54px] items-center">
        {cities.map((city) => (
          <div
            key={city}
            className="flex items-center gap-1 bg-white dark:bg-zinc-700 rounded-full px-3 py-1 text-sm"
          >
            {city}
        ...
        <Input
          ...
          placeholder={
            cities.length === 0 ? "Add cities and press Enter..." : ""
          }
...
const AddVendorForm = ({
  ...
  onNext: (vendorName: string) => void;
  vendorToEdit?: any | null;
}) => {
  const [selectedDays, setSelectedDays] = useState(
    vendorToEdit?.availability.days || ["M", "T", "W", "Th", "F"],
  );

  const [logo, setLogo] = useState<File | null>(null);
  const [companyName, setCompanyName] = useState(
    vendorToEdit?.companyName || "",
  );
  ...
  const [, setCinCert] = useState<File | null>(null);
  const [, setGstCert] = useState<File | null>(null);
  const [, setBrochure] = useState<File | null>(null);

  const [cinCertName, setCinCertName] = useState(
    vendorToEdit?.cinCertificate || "",
  );
  const [gstCertName, setGstCertName] = useState(
    vendorToEdit?.gstCertificate || "",
  );
  const [brochureName, setBrochureName] = useState(
    vendorToEdit?.brochure || "",
  );

  const [gstNumber, setGstNumber] = useState(vendorToEdit?.gstNumber || "");
  const [serviceableCities, setServiceableCities] = useState(
    vendorToEdit?.serviceableCities || ["Bengaluru"],
  );
  const [availableTimeFrom, setAvailableTimeFrom] = useState(
    vendorToEdit?.availability.time.split(" - ")[0] || "",
  );
  const [availableTimeTo, setAvailableTimeTo] = useState(
    vendorToEdit?.availability.time.split(" - ")[1] || "",
  );
  const [bankName, setBankName] = useState(
    vendorToEdit?.accountDetails.bankName || "",
  );
  const [accountHolder, setAccountHolder] = useState(
    vendorToEdit?.accountDetails.accountHolder || "",
  );
  const [accountNumber, setAccountNumber] = useState(
    vendorToEdit?.accountDetails.accountNumber || "",
  );
  const [confirmAccountNumber, setConfirmAccountNumber] = useState(
    vendorToEdit?.accountDetails.accountNumber || "",
  );
  const [ifscCode, setIfscCode] = useState(
    vendorToEdit?.accountDetails.ifscCode || "",
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full">
      <ScrollArea className="flex-1 p-6 no-scrollbar">
        <div className="space-y-6">
          <div className="flex gap-4 items-center">
            <label
              htmlFor="logo-upload"
              className="w-24 h-24 bg-input rounded-2xl border border-stone-300 flex items-center justify-center cursor-pointer hover:bg-stone-200"
            >
              {logo ? (
                <Image
            ...
            <div className="space-y-4 flex-1">
              <FloatingLabelInput
                ...
                value={companyName}
                onChange={handleTextOnlyChange(setCompanyName)}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FileUploadField
              ...
              onChange={handleFileChange(setCinCert, setCinCertName)}
              fileName={cinCertName}
              onRemove={handleRemoveFile(setCinCert, setCinCertName)}
            />
            <FileUploadField
              id="gst-cert"
              label="GST Certificate"
              onChange={handleFileChange(setGstCert, setGstCertName)}
              fileName={gstCertName}
              onRemove={handleRemoveFile(setGstCert, setGstCertName)}
            />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FloatingLabelInput
              ...
              value={gstNumber}
              onChange={handleAlphanumericChange(setGstNumber)}
            />
            <FileUploadField
              id="brochure"
              label="Product Brochure"
              onChange={handleFileChange(setBrochure, setBrochureName)}
              fileName={brochureName}
              onRemove={handleRemoveFile(setBrochure, setBrochureName)}
            />
          </div>
          <ServiceableCityInput
            cities={serviceableCities}
            setCities={setServiceableCities}
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FloatingLabelInput
                id="available-time-from"
                name="available-time-from"
                type="text"
              ...
              />
              <FloatingLabelInput
                id="available-time-to"
                name="available-time-to"
                type="text"
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FloatingLabelInput
              ...
              value={bankName}
              onChange={handleTextOnlyChange(setBankName)}
            />
            <FloatingLabelInput
              id="account-holder"
              ...
              value={accountHolder}
              onChange={handleTextOnlyChange(setAccountHolder)}
            />
            <FloatingLabelInput
              id="account-number"
              ...
              value={accountNumber}
              onChange={handleNumberOnlyChange(setAccountNumber)}
            />
            <FloatingLabelInput
              ...
              onChange={handleNumberOnlyChange(setConfirmAccountNumber)}
            />
            <FloatingLabelInput
              id="ifsc-code"
              label="IFSC Code*"
              ...
              value={ifscCode}
              onChange={handleAlphanumericChange(setIfscCode)}
            />
            <FloatingLabelInput
              id="upi-id"
              label="UPI ID"
      ...
      <div className="p-6 mt-auto border-t md:border-0 md:flex md:justify-end">
        <Button
          type="submit"
          className="w-full md:w-auto h-[54px] px-10 py-3.5 bg-primary rounded-full text-lg"
        >
...
const EditMaterialForm = ({
  ...
  const [price, setPrice] = useState(
    material.price.replace("₹", "").replace(",", ""),
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 space-y-6">
      <h3 className="text-lg font-medium">
        Edit Material:{" "}
        <span className="font-semibold text-primary">{material.name}</span>
      </h3>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <div className="w-36 h-36 bg-input rounded-[10px] border border-stone-300 flex items-center justify-center">
            <Image
              ...
              height={100}
              alt={material.name}
              className="rounded-[10px] border border-stone-300"
        ...
        <div className="space-y-6">
          ...
          <FloatingLabelInput
            ...
            value={price}
            onChange={handleNumberOnlyChange(setPrice)}
          />
          <div className="space-y-2">
            <Label
              htmlFor="description"
              className="text-zinc-900 dark:text-white text-lg font-medium px-2"
            >
              Description*
            </Label>
            <Textarea
              ...
              className="h-36 bg-background rounded-3xl"
  ...
  return (
    <div className="flex flex-col h-full">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <label
                  ...
                  className="w-36 h-36 bg-input rounded-[10px] border border-stone-300 flex items-center justify-center cursor-pointer hover:bg-stone-200"
                >
                  {productImage ? (
                    <Image
          ...
          <div className="border-t pt-6 mt-6">
            ...
            <div className="space-y-4">
              {materials.map((material) => (
                <div
                  key={material.id}
                  className="flex items-center gap-4 p-4 rounded-[10px] border border-stone-300"
                >
                  <Image
                    src={material.image}
                    width={100}
      ...
      <div className="p-6 mt-auto border-t flex flex-col-reverse md:flex-row justify-between items-center gap-4 shrink-0 bg-card rounded-b-[20px]">
        <Button
          type="button"
          className="w-full md:w-auto h-14 text-lg rounded-full"
          onClick={onVendorAdded}
        >
          Save
        </Button>
        ...
        >
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {step === "addVendor" ? (
              ...
              <AddMaterialForm
                vendorName={vendorName || vendorToEdit.companyName}
                onBack={handleBack}
                onVendorAdded={handleVendorAdded}
                initialMaterials={vendorToEdit?.materials}
              />
            )}
          </div>
        </SheetContent>
