...
const DailyUpdateCard = ({ update, onUpdate }: { update: DailyUpdate, onUpdate: (update: DailyUpdate) => void }) => {
    ...
    const formattedDate = new Date(update.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
        ...
        <Card className="rounded-[40px] border p-6">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Uploaded by {update.uploadedBy}</p>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
                {update.photos.map((photo, index) => (
                    <div key={index} className="relative aspect-square">
                        <Image src={photo} alt={`Update photo ${index+1}`} layout="fill" objectFit="cover" className="rounded-lg"/>
                    </div>
                ))}
            </div>
            {update.status === 'pending' && (
                <div className="flex gap-4 mt-4">
                    <Button onClick={handleReject} variant="outline" className="flex-1 rounded-full text-destructive hover:bg-destructive/10 hover:text-destructive h-[54px] border-0 text-base md:text-lg">Reject</Button>
...
export function DailyUpdatesSheet({ isOpen, onClose, dailyUpdates, onUpdate }: DailyUpdatesSheetProps) {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredUpdates = useMemo(() => {
    const sortedUpdates = [...dailyUpdates].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
          ...
          <div className="p-6 space-y-4">
             {filteredUpdates.length > 0 ? (
                 filteredUpdates.map(update => (
                    <DailyUpdateCard key={update.id} update={update} onUpdate={onUpdate} />
                ))
             ) : (
                <p className="text-muted-foreground col-span-full text-center py-10">No daily updates found.</p>
             )}
          </div>
        </ScrollArea>
      </SheetContent>
