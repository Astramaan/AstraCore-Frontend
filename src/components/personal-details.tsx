...
const EditProfileForm = React.memo(
  ({
    ...
    const [isRoleChangeConfirmOpen, setIsRoleChangeConfirmOpen] =
      useState(false);
    ...
    return (
      <form onSubmit={handleSubmit} className="flex flex-col h-full">
        ...
        <div className="px-6 py-4 border-t flex justify-end gap-2 bg-card rounded-b-[50px] mt-auto shrink-0">
          <Button
            type="submit"
            className="w-full md:w-auto h-14 px-10 rounded-full text-lg"
          ...
          <AlertDialogContent className="max-w-md rounded-[50px]">
            <AlertDialogHeader className="items-center text-center">
              ...
              <AlertDialogDescription className="text-lg text-muted-foreground">
                Changing the role of a Super Admin can have significant security
                implications. Are you sure you want to proceed?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter className="sm:justify-center gap-4 pt-4">
              <AlertDialogCancel
                onClick={cancelRoleChange}
                className="w-40 h-14 px-10 rounded-[50px] text-lg font-medium text-foreground border-none hover:bg-primary/10 hover:text-primary"
              >
                Cancel
              </AlertDialogCancel>
              <AlertDialogAction
                onClick={confirmRoleChange}
                className="w-40 h-14 px-10 bg-primary rounded-[50px] text-lg font-medium text-white hover:bg-primary/90"
              >
                Confirm
              </AlertDialogAction>
            </AlertDialogFooter>
...
export function PersonalDetails({ memberId }: PersonalDetailsProps) {
  ...
  if (!member) {
    return (
      <Card className="rounded-[50px] p-10">
        <CardContent className="p-0">Could not load user details.</CardContent>
      </Card>
    );
  }
  ...
  return (
    <DialogOrSheet open={isEditing} onOpenChange={handleOpenChange}>
          ...
          <div className="flex flex-col md:flex-col lg:flex-row md:items-start gap-4 md:gap-8">
                ...
                <div className="flex flex-col gap-2 md:items-end">
                  ...
                  <ChangePasswordDialog
                    ...
                    trigger={
                      <Button
                        variant="outline"
                        className="md:w-56 h-12 rounded-full bg-card text-foreground hover:bg-muted text-base font-medium"
                      >
                        Change Password
                      </Button>
                    }
                  ...
                  {isSuperAdmin && (
                    <Button
                      className="md:w-56 h-12 rounded-full text-primary text-base font-medium bg-primary/10 border border-primary hover:bg-primary/20"
                      onClick={() => setIsBrandingSheetOpen(true)}
                    >
            ...
            <div className="flex-1 w-full grid grid-cols-2 gap-y-4 gap-x-4 md:gap-x-8">
              <div className="space-y-1">
                ...
                <p className="text-foreground text-base md:text-lg leading-tight truncate">
                  {member.name}
                </p>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <p className="text-primary text-base md:text-lg leading-tight truncate">
                  {member.team}
                </p>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1 col-span-2 md:col-span-1">
                ...
                <p className="text-foreground text-base md:text-lg leading-tight">
                  {initialMemberData.address}
                </p>
              </div>
            </div>
            {isOwner && (
      ...
      <BrandingSheet
        isOpen={isBrandingSheetOpen}
        onOpenChange={setIsBrandingSheetOpen}
      />
    </DialogOrSheet>
  );
}
