...
export const TaskCard = ({
  ...
  return (
    ...
    >
      <Card
        className="w-full h-44 rounded-[40px] flex flex-col justify-between p-6 cursor-pointer hover:shadow-lg transition-shadow bg-white dark:bg-card"
        onClick={onClick}
      >
        <div className="flex-1 flex flex-col">
          ...
          <p className="text-base text-card-foreground mt-2 truncate flex-grow">
            {task.description}
          </p>
          {task.status &&
          ...
          <div className="flex items-center">
            <div className="flex -space-x-2">
              <Avatar className="w-6 h-6 border-2 border-white dark:border-card">
                <AvatarImage
                  src="https://picsum.photos/seed/avatar1/25/25"
                  data-ai-hint="person portrait"
                />
              </Avatar>
              <Avatar className="w-6 h-6 border-2 border-white dark:border-card">
                <AvatarImage
                  src="https://picsum.photos/seed/avatar2/25/25"
                  data-ai-hint="person portrait"
                />
              </Avatar>
            </div>
            <Badge
              variant="outline"
              className="ml-4 bg-zinc-100 dark:bg-zinc-800 border-zinc-100 dark:border-zinc-800 text-zinc-900 dark:text-zinc-200"
            >
              {task.category}
            </Badge>
          </div>
          <div className="text-right flex items-center gap-2">
            <p className={cn("text-sm font-medium", dateColor)}>
              Due: {formattedDate}
            </p>
          </div>
        </div>
