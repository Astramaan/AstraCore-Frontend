...
export function InPersonConsultationDialog({
  isOpen,
  onOpenChange,
  initialView,
  onBookingSuccess,
}: InPersonConsultationDialogProps) {
  const [view, setView] = useState<
    "initial" | "office" | "home" | "online" | "in-person" | null
  >(initialView);
          ...
          {view === "in-person" && (
            <div className="flex flex-col items-center gap-8">
              <div className="text-center w-full">
                <Button
                  className="w-full h-[54px] rounded-full text-lg text-primary-foreground leading-tight"
                  onClick={() => setView("office")}
                >
                  Visit Our Office
                </Button>
                <p className="text-stone-400 text-sm mt-2">
                  Meet with a Project specialist at our office.
                </p>
              </div>
              <div className="text-center w-full">
                <Button
                ...
                <p className="text-stone-400 text-sm mt-2">
                  A project specialist will visit your location.{" "}
                </p>
              </div>
            </div>
          )}
