...
const allStages: CustomStage[] = [
  {
    ...
    description: "Design presentation details",
    priority: "Low",
  },
  {
    id: 2,
    title: "1% Of Over all Quote",
  ...
  },
  {
    ...
    createdAt: new Date().toISOString(),
    description: "Grid marking details",
    priority: "Medium",
  },
  {
    id: 6,
    ...
    createdBy: "System",
    createdAt: new Date().toISOString(),
    description: "20% milestone payment",
    priority: "High",
  },
];
...
const StageCard = ({
  ...
  stage: CustomStage;
  onReopen?: (stage: CustomStage) => void;
  onPayment?: (stageId: number) => void;
  className?: string;
}) => {
  const { user } = useUser();
      ...
      <CardContent className="p-0">
        <div className="w-full space-y-4">
          {displayStages.map((stage) => (
            <StageCard key={stage.id} stage={stage} onPayment={onPayment} />
          ))}
        </div>
      </CardContent>
    </Card>
