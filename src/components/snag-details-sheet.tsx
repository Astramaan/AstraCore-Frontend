    ...
    <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1">
        <div className="p-6 space-y-6">
                  ...
                  {snag.images.map((image, index) => (
                    <CarouselItem key={index}>
                      <div className="w-full aspect-video relative">
                        <Image
                          src={image}
                          alt={`${snag.title} - image ${index + 1}`}
                          layout="fill"
                          className="rounded-lg object-cover"
                          data-ai-hint="defect photo"
                        />
                      </div>
                    </CarouselItem>
                  ))}
            ...
            {isEditing ? (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-14 rounded-full bg-background"
                />
              </div>
            ) : (
            ...
            {isEditing ? (
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={handleInputChange}
                  className="text-muted-foreground min-h-[54px] rounded-3xl bg-background"
                />
              </div>
            ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <DetailRow
              label="Project"
              value={`${snag.projectName} (${snag.projectId})`}
            />
            ...
            <DetailRow
              ...
              value={
                <Badge
                  className={cn("text-base", snag.statusColor)}
                  variant="outline"
                >
                  {snag.status}
                </Badge>
      ...
      {isEditing ? (
        <div className="p-4 border-t mt-auto flex justify-end gap-2 shrink-0">
          <Button
            onClick={handleSave}
            className="w-full md:w-auto md:flex-initial rounded-full h-14 px-10 text-lg"
          >
...
export function SnagDetailsSheet({
  isOpen,
  onClose,
  snag,
  onDelete,
