...
export const DetailField = ({ label, value, isEditing, onChange, name, placeholder, type = 'text' }: DetailFieldProps) => {
    return (
        <div className="space-y-1">
            <Label htmlFor={name} className="text-base text-grey-1 px-2">{label}</Label>
            {isEditing ? (
                 <Input
                    id={name}
                    name={name}
