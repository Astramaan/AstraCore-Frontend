...
const navItems = [
    ...
    { href: "/platform/subscription-management", icon: HandCoins, label: "Subscription Ma.." },
                        ...
                        return (
                            <Link href={item.href} key={item.label} title={item.label} className="flex-shrink-0">
                                <div className={cn(
                                    ...
                                    <div className={`p-4 rounded-full ${isActive ? 'bg-primary' : 'bg-white'}`}>
                                        <item.icon className={`h-6 w-6 ${isActive ? 'text-white' : 'text-black'}`} />
                                    </div>
                                    <span className={`text-xs font-medium ${isActive ? 'text-primary' : 'text-gray-600'}`}>{item.label}</span>
                                </div>
                            </Link>
                        );
                    })}
