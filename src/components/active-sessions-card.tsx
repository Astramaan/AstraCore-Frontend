    ...
    <Card className="rounded-[50px]">
      ...
      <CardContent className="px-6 pb-6">
        {sessions.map((session) => (
          <div
            key={session.device}
            className="flex justify-between items-center py-4 border-b last:border-b-0 last:pb-0"
          >
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  variant="destructive"
                  className="h-14 px-10 rounded-full bg-background hover:bg-destructive/10 text-red-600 text-lg font-medium hidden md:flex"
                >
                  Remove
                </Button>
              </AlertDialogTrigger>
              <AlertDialogTrigger asChild>
                <Button
                  size="icon"
                  variant="ghost"
                  className="md:hidden p-3.5 rounded-full bg-destructive/10 text-destructive hover:bg-destructive/20"
                >
                  <X className="h-6 w-6" />
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent className="max-w-md rounded-[50px]">
                <AlertDialogHeader className="items-center text-center">
                  ...
                  <AlertDialogDescription className="text-lg text-muted-foreground">
                    You will be logged out from this device and will need to
                    sign in again.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter className="flex-row justify-center items-center gap-4 pt-4">
                  <AlertDialogCancel className="flex-1 h-14 px-10 rounded-[50px] text-lg font-medium text-foreground border-none bg-zinc-100 hover:bg-zinc-200 hover:text-black">
                    Cancel
                  </AlertDialogCancel>
                  <AlertDialogAction
                    onClick={() => handleRemove(session.device)}
                    className="flex-1 h-14 px-10 bg-red-600 rounded-[50px] text-lg font-medium text-white hover:bg-red-700"
                  >
                    Remove
                  </AlertDialogAction>
                </AlertDialogFooter>
