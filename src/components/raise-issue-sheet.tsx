...
const RaiseIssueForm = ({ stageTitle, onClose, onSubmit }: { stageTitle: string, onClose: () => void, onSubmit: (issueInfo: IssueInfo) => void }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
            <ScrollArea className="flex-1 p-6 no-scrollbar">
                <div className="space-y-6">
                    <div className="space-y-2">
                        <Label htmlFor="issue-comments" className={cn("text-lg font-medium", comments ? 'text-grey-1' : 'text-zinc-900')}>Comments</Label>
                        <Textarea 
                            id="issue-comments" 
            ...
            <div className="p-6 mt-auto border-t md:border-0 md:flex md:justify-end">
                <Button type="submit" className="w-full md:w-auto md:px-14 h-[54px] text-lg rounded-full">
                    Submit Issue
                </Button>
            </div>
        </form>
...
export function RaiseIssueSheet({ isOpen, onClose, stageTitle, onSubmit }: RaiseIssueSheetProps) {
    return (
        <Sheet open={isOpen} onOpenChange={onClose}>
            ...
            >
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    <RaiseIssueForm stageTitle={stageTitle} onClose={onClose} onSubmit={onSubmit} />
                </div>
            </SheetContent>
        </Sheet>
    );
