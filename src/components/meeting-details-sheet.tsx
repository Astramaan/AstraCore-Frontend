    ...
    <div className="flex flex-col h-full">
      <div className="p-6 space-y-6 overflow-y-auto flex-1 no-scrollbar">
        <div className="space-y-8">
          ...
          <DetailRow label={meeting.type === 'lead' ? 'Lead ID' : 'Client ID'} value={<Badge variant="outline" className="bg-zinc-100 dark:bg-zinc-800 border-zinc-100 dark:border-zinc-800 text-zinc-900 dark:text-zinc-200 text-base">{meeting.id}</Badge>} />
          ...
          <DetailRow label="Meeting Link" value={
            <a href={`https://${meeting.link}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-foreground font-medium hover:underline">
