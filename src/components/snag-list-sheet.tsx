...
const SnagCard = ({ snag, onClick }: { snag: Snag, onClick: (snag: Snag) => void }) => (
  <Card className="rounded-[40px] border p-6 cursor-pointer hover:shadow-lg" onClick={() => onClick(snag)}>
    <div className="flex justify-between items-start">
      <div>
        <p className="font-semibold text-lg">{snag.title}</p>
        <p className="text-sm text-muted-foreground">{snag.createdBy}</p>
      </div>
      <Badge className={cn("text-base", snag.statusColor)} variant="outline">{snag.status}</Badge>
    </div>
    <p className="text-sm text-muted-foreground mt-2 line-clamp-2">{snag.description}</p>
    <Separator className="my-4"/>
    <div className="flex justify-between items-center">
      <p className="text-xs text-muted-foreground">{new Date(snag.createdAt).toLocaleDateString()}</p>
      <div className="flex items-center gap-2">
        {snag.images.map((img, i) => (
          <Image key={i} src={img} width={40} height={40} alt="Snag image" className="rounded-md"/>
        ))}
      </div>
    </div>
  </Card>
...
export function SnagListSheet({ isOpen, onClose, snags, projectId }: SnagListSheetProps) {
  ...
  const filteredSnags = useMemo(() => {
    ...
    return projectSnags.filter(snag => 
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredSnags.length > 0 ? (
                  filteredSnags.map(snag => (
                    <SnagCard key={snag.id} snag={snag} onClick={handleSnagClick} />
                  ))
              ) : (
                  <p className="text-muted-foreground col-span-full text-center py-10">No snags found for this project.</p>
              )}
            </div>
          </ScrollArea>
        </SheetContent>
