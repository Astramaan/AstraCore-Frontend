...
const FileItem = ({
  ...
  file: File;
  onFileClick: (file: File | FileVersion) => void;
}) => (
    ...
    <AccordionItem value={file.id} className="border-b-0">
      <div className="flex items-center gap-4 py-3 group">
        ...
        >
          ...
          <div className="flex-1">
            <p className="text-base text-foreground font-medium group-hover:text-primary">
              {file.name}
            </p>
        ...
        <AccordionContent>
          <div className="pl-10 space-y-2 border-l ml-3">
            {file.history.map((version, index) => (
              <div
                key={index}
                className="flex items-center gap-4 py-2 cursor-pointer group pl-4"
              >
                <div className="flex-1" onClick={() => onFileClick(version)}>
                  <p className="text-sm text-foreground font-medium group-hover:text-primary">
                    {version.name}
                  </p>
...
export const ProjectFilesCard = ({ phases }: ProjectFilesCardProps) => {
  const [selectedFile, setSelectedFile] = useState<File | FileVersion | null>(
    null,
  );
      ...
      <Card className="rounded-[50px] p-0 bg-transparent text-card-foreground shadow-none border-none">
        <CardContent className="p-0">
          <Accordion type="single" collapsible className="w-full space-y-4">
            {phases.map((phase) => {
              const totalFiles = phase.stages.reduce(
                (acc, stage) => acc + (stage.documents?.length || 0),
                0,
              );
                        ...
                        (stage) =>
                          ...
                          stage.documents.length > 0 && (
                            <div key={stage.name} className="pl-4">
                              <h4 className="font-semibold text-md text-muted-foreground my-2">
                                {stage.name}
                              </h4>
